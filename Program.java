...
в котором будет происходить вывод всей информации о товаре, так же реализовать конструкторы, принимающие все поля.
...
public class Program {
    ...
    public static void getData(LinkedList<Product> foods, LinkedList<Product> drink, LinkedList<Product> hygieneGoods, LinkedList<Product> childrenGoods) {
        System.out.print("""
                Уважаемый пользователь,
                укажите категорию товара, о которых нужно получить сведения:
                Продукты питания       - нажмите 1
                Напитки                - нажмите 2
                Предметы гигиены       - нажмите 3
                Детские товары         - нажмите 4
                Все товары             - нажмите 5
                Ваше решение: \s""");
        Scanner aScanner = new Scanner(System.in);
        int decisionNumber = aScanner.nextInt();
        if (decisionNumber == 1) {
            System.out.print("\nВ МАГАЗИНЕ ИМЕЮТСЯ СЛЕДУЮЩИЕ ПРОДУКТЫ ПИТАНИЯ:");
            for (Product element : foods) {
                System.out.printf("\n%s", element);
            }
        }
        if (decisionNumber == 2) {
            System.out.print("\nВ МАГАЗИНЕ ИМЕЮТСЯ СЛЕДУЮЩИЕ НАПИТКИ:");
            for (Product element : drink) {
                System.out.printf("\n%s", element);
            }
        }
        if (decisionNumber == 3) {
            System.out.print("\nВ МАГАЗИНЕ ИМЕЮТСЯ СЛЕДУЮЩИЕ ПРОДУКТЫ ЛИЧНОЙ ГИГИЕНЫ:");
            for (Product element : hygieneGoods) {
                System.out.printf("\n%s", element);
            }
        }
        if (decisionNumber == 4) {
            System.out.print("\nВ МАГАЗИНЕ ИМЕЮТСЯ СЛЕДУЮЩИЕ ДЕТСКИЕ ТОВАРЫ:");
            for (Product element : childrenGoods) {
                System.out.printf("\n%s", element);
            }
        }
        if (decisionNumber == 5) {
            System.out.println("\nВ МАГАЗИНЕ ИМЕЮТСЯ СЛЕДУЮЩИЕ ТОВАРЫ:");
            System.out.println("\nПРОДУКТЫ ПИТАНИЯ:");
            for (Product element : foods) {
                System.out.printf("\n%s", element);
            }
